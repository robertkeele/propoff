                            ...
                            <div>
                                ...
                                <dl class="space-y-3">
                                        ...
                                        <dd class="mt-1">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                                :class="{
                                                    ...
                                                    'bg-yellow-100 text-yellow-800': event.status === 'locked',
                                    ...
                                    <div>
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900">{{ event.questions.length }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500">Total Submissions</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ event.submissions_count || 0 }}</dd>
                                    </div>
                                    <div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="mb-4">
                            ...
                            <p class="text-sm text-gray-500 mt-1">This event contains {{ event.questions.length }} {{ event.questions.length === 1 ? 'question' : 'questions' }}</p>
                        </div>

                        <div v-if="event.questions.length === 0" class="text-center py-8 text-gray-500">
                            No questions available yet.
                        </div>

                        <div v-else class="space-y-3">
                            <div
                                v-for="(question, index) in event.questions.slice(0, 3)"
                                :key="question.id"
                            ...
                            <div v-if="event.questions.length > 3" class="text-center text-sm text-gray-500 pt-2">
                                ... and {{ event.questions.length - 3 }} more {{ event.questions.length - 3 === 1 ? 'question' : 'questions' }}
                            </div>
                        </div>
                    </div>
                </div>
