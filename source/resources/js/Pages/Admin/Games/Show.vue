...
const updateStatus = (newStatus) => {
    if (confirm(`Are you sure you want to change the game status to "${newStatus}"?`)) {
        statusUpdating.value = true;
        router.post(
            route('admin.games.updateStatus', props.game.id),
            { status: newStatus },
            {
                onFinish: () => {
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900">Game Information</h3>
                                <span
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <p class="mt-1 text-gray-900">{{ game.description || 'No description' }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Event Date</p>
                                <p class="mt-1 text-gray-900">{{ formatDate(game.event_date) }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Lock Date</p>
                                <p class="mt-1 text-gray-900">
                                    {{ game.lock_date ? formatDate(game.lock_date) : 'Not set' }}
                                </p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Created</p>
                                <p class="mt-1 text-gray-900">{{ formatDate(game.created_at) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 border-t pt-6">
                            ...
                            <div class="flex space-x-2">
                                <button
                                    ...
                                    :disabled="game.status === status || statusUpdating"
